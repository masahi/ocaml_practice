...
let idyn : 'c index -> int code = function I (x,bv) ->
...
type 'c garr = 'c index -> float code
;;
...
let ( <-@ ) : float array code -> 'c garr ->
              ('c index -> unit code) = fun a b j ->
  gset0 a (b j) j;;
...
let t1r =
  ...
  (Runnative.run t1c) (Array.length a1) a1 b1; b1
;;
(* Check the result *)
let _ =
  assert (t1r =
...
let t2r =
  let b1 = Array.make (Array.length a1) (-100.0) in
  (Runnative.run t2c) (Array.length a1) a1 b1; b1
;;
let _ = assert(t1r = t2r);;
(* The sample result is the same *)

type 'c stencil =
  'c bounded *				(* The base of the index i *)
  (int * float ref code) list	(* Cache for a[i+j] for a range of j.
...
let stencil_slide : 'c stencil -> 'c garr -> unit code =
  fun (iv,table) a ->
...
let stencil_memo q p (a_base : 'c garr) : 'c garr  =
  ...
  let make_ref ix =			(* outside the loop *)
    shift q (fun k ->			(* scope extrusion may happen! *)
      ...
      .<let s = ref .~ar in .~(k .<s>.)>.) in
					(* Cache access  *)
  let aref (inited, (iv',table)) ((I (j, _)) as ix) =
    try List.assoc j table
    with Not_found ->
  ...
  let check ((I (j,iv)) as ix) =
    ...
    | Some ((_,(iv',_)) as st) when same_bounded iv' iv -> aref st ix
    | Some (_,(iv',table)) when successive_bounded iv' iv ->
       shift p (fun k ->
...
let t3r =
  let b1 = Array.make (Array.length a1) (-100.0) in
  (Runnative.run t3c) (Array.length a1) a1 b1; b1
;;
let _ = assert (t1r = t3r);;
(* The sample result is the same *)
...
let t31r =
  let b1 = Array.make (Array.length a1) (-100.0) in
  (Runnative.run t31c) (Array.length a1) a1 b1; b1
;;
let _ = assert(t1r = t31r);;
...
let t20r =
  ...
  (Runnative.run t1c) n a1 b1;
  let c1 = Array.make n (-100.0) in
  (Runnative.run t1c) n b1 c1;
c1
;;
let _ = assert(t20r =
...
let t21r =
  let b1 = Array.make (Array.length a1) (-100.0) in
  (Runnative.run t21c) (Array.length a1) a1 b1; b1
;;
let _ = assert(t20r = t21r);;
...
let t25r =
  let b1 = Array.make (Array.length a1) (-100.0) in
  (Runnative.run t25c) (Array.length a1) a1 b1; b1
;;
let _ = assert(t20r = t25r);;
...
let _ = assert (t20r = stencil_hand_r);;
