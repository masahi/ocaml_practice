...
type 'a newfloats_aa = Code of rat * float code * float code
    ...
    let array_read arr n =
      let rec gl m l =
        if m = n then ret l
          ...
          let sm = m+1 in
          let* c =  abstract_simple (CArray1D.getL arr m, CArray1D.getL arr sm) in
          gl (m+2) (l @ [c])
      in gl 0 [] in
...
(*  (rx,ix) +/- _complex_fy*(ry,iy) and set the resulting
    factor to fx
...
(* This one is monadic: it may need to let-bind something
   ...
   The second method is ed by do_linear
*)
...
let test_ad dir lst =
  ...
  print_code Format.std_formatter tranc; print_newline();
